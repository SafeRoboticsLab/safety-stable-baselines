...
if __name__ == "__main__":
    # ---------- argument parsing ----------
    parser = argparse.ArgumentParser(description="Train PPO with SafetySAC1P safety filter on CarCircle")
    ...
    parser.add_argument("--safety-model-path", type=str, 
                        default="./experiments/20250930_0123_SafetySAC_CarCircle2_pillar/final/car_circle2.zip",
                        help="Path to trained SafetySAC1P model for safety filtering")
    ...
    base_run_name = f"PPO_CarCircle2_WithFilter_{epsilon_str}"
    ...
    if not os.path.exists(safety_model_path):
        ...
        print("Please train a SafetySAC1P model first using car_circle_safety_train.py")
    ...
    # ---------- model ----------
    # Standard PPO (same as naive training) with SafetySAC1P safety filter
    model = PPO(
        policy="MlpPolicy",
        env=env,
    ...
    ckpt_cb = CheckpointCallback(
        ...
        save_path=ckpt_dir,
        name_prefix="ppo_car_circle2_withfilter",
    ...
    # ---------- final save ----------
    final_path = os.path.join(final_dir, f"car_circle2_ppo_withfilter_{epsilon_str}")
