...
class SafetyRolloutFilter:
    ...
    def _create_rollout_env(self):
        ...
        try:
            ...
            rollout_env = safety_gymnasium.make("SafetyCarGoal1-v0", render_mode=None)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--safety-model-path", type=str, 
                        default="./experiments/20251008_0050_SafetySAC_CarGoal1_Pillar_2M/checkpoints/safety_sac_car_goal1_pillar_1400000_steps.zip",
    ...
    # ---------- paths ----------
    base_run_name = f"SAC_CarGoal1_WithRolloutFilter_h{HORIZON}_vt{VELOCITY_THRESHOLD}"
    ...
    base_env = safety_gymnasium.make("SafetyCarGoal1-v0", render_mode=None)
    ...
    # Separate eval env with same rollout filter
    ...
    ckpt_cb = CheckpointCallback(
        save_freq=10_000,
        save_path=ckpt_dir,
        name_prefix="sac_car_goal1_withrolloutfilter",
    ...
    # ---------- final save ----------
    final_path = os.path.join(final_dir, f"car_goal1_sac_withrolloutfilter_h{HORIZON}_vt{VELOCITY_THRESHOLD}")
    model.save(final_path)
    print(f"Training complete! Saved final SAC+RolloutFilter (h={HORIZON}, vt={VELOCITY_THRESHOLD}) model to {final_path}.zip")
