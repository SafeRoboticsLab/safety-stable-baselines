...
from safety_gymnasium.safety_envs.fixed_position_wrapper import (
    make_circle_env_with_fixed_position, 
    get_challenging_position,
...
def replay_model(model_path: str, agent: str = "Car", level: int = 2, 
    ...
    env = make_circle_env_with_fixed_position(
        agent=agent,
        level=level,
        position=position,
...
def main():
    ...
    parser.add_argument("--position", "-p", type=float, nargs=2, metavar=('X', 'Y'),
                        help="Fixed starting position (x, y). Overrides --difficulty.")
    parser.add_argument("--difficulty", type=str, 
                        choices=["near_border", "very_close_to_border", "almost_unsafe"],
                        help="Predefined challenging position difficulty")
    parser.add_argument("--position-index", type=int, default=0, choices=[0, 1, 2, 3],
                        help="Position index for predefined difficulties (0-3 for different sides)")
    ...
    # Convert position argument to tuple
    fixed_position = tuple(args.position) if args.position else None
