...
# Safety SB3
from safety_sb3 import SafetyPPO, SafetySAC, SafetyA2C, SafetyDQN
...
def get_wrapper_class(hyperparams: dict[str, Any],
                      key: str = "env_wrapper") -> Optional[Callable[[gym.Env], gym.Env]]:
    """
...
def get_hf_trained_models(organization: str = "sb3",
                          check_filename: bool = False) -> dict[str, tuple[str, str]]:
    """
    ...
    for model in models:
        ...
        if check_filename:
            ...
            if maybe_siblings and not any(
                f.rfilename == model_name.filename for f in maybe_siblings
            ):
