"""Train SafetySAC (reach-avoid V + fallback pi_safe) on ContactSafetyEnv.

Same CLI/training shape as train.py, just pointed at the MuJoCo-contact env instead of the fast
analytic one -- see mujoco_env.py for what's different (contact + slam margins, real physics).
Expect far fewer steps/sec than train.py's analytic env; start small and scale up.

  python -m vault.train_contact --steps 3000      # smoke (validate API + short train)
  python -m vault.train_contact --steps 300000    # full, once the smoke run looks sane
"""
...
def main():
    ap = argparse.ArgumentParser(description="Train SafetySAC on the MuJoCo contact reach-avoid env")
    ...
    ap.add_argument("--save", type=str, default=str(C.MODELS / "contact_safety_sac"))
    args = ap.parse_args()

    env = ContactSafetyEnv(seed=0)
